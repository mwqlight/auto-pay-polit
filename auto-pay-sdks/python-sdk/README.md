...
简洁易用的支付服务 Python SDK，支持同步/异步操作，完整类型提示，内置重试机制。

## ✨ 特性

- 🚀 **简洁易用**: 一行代码创建支付，链式调用更优雅
- 🔄 **同步/异步**: 支持同步和异步两种编程模式
- 🔒 **类型安全**: 完整的类型提示，IDE智能提示支持
- 🛡️ **错误处理**: 完善的异常体系，精准错误定位
- ⚡ **高性能**: 连接池复用，支持请求并发
- 🔄 **智能重试**: 网络异常自动重试，可配置策略
- 📊 **数据完整**: 所有API接口全覆盖
- 🧪 **测试完善**: 单元测试覆盖率>90%
...
async def async_example():
    ...
    async with AsyncHttpClient(config) as http_client:
        ...
        for i, result in enumerate(results):
            print(f"订单{i+1}: {result.get('data', {}).get('payment_id')}")
...
print(f"健康渠道数: {health_summary['healthy_channels']}")

# 批量退款
batch_refunds = [
    {"payment_id": "pay_123", "amount": 50.00, "reason": "商品质量问题"},
    {"payment_id": "pay_124", "amount": 30.00, "reason": "重复下单"}
]

result = client.batch_refund(batch_refunds)
print(f"批量退款结果: {result}")
```
...
| `api_key` | str | - | API密钥 |
| `secret_key` | str | - | 密钥 |
| `environment` | Environment | SANDBOX | 环境 |
| `base_url` | str | - | 基础URL |
| `timeout_seconds` | int | 30 | 请求超时 |
| `connect_timeout_seconds` | int | 10 | 连接超时 |
| `read_timeout_seconds` | int | 30 | 读取超时 |
| `max_retries` | int | 3 | 最大重试次数 |
| `base_delay` | float | 1.0 | 重试基础延迟 |
| `max_delay` | float | 60.0 | 最大延迟 |
| `rate_limit_requests` | int | 100 | 请求频率限制 |
| `rate_limit_window` | int | 60 | 频率限制窗口(秒) |
| `proxy_url` | str | None | 代理URL |
| `user_agent` | str | None | 用户代理 |
| `enable_logging` | bool | True | 是否启用日志 |
| `connection_pool_size` | int | 100 | 连接池大小 |
| `connection_pool_size_per_host` | int | 30 | 每主机连接池大小 |
...
from autopay import (
    ...
    generate_payment_id,    # 生成支付号
    generate_signature,     # 生成签名
    verify_signature,       # 验证签名
    format_decimal,         # 格式化小数
    format_currency,        # 格式化货币
    mask_sensitive_data,    # 掩码敏感数据
...
├── test_refund.py          # 退款服务测试
├── test_channel.py         # 渠道服务测试
├── test_account.py         # 账户服务测试
├── test_system.py          # 系统服务测试
├── test_webhook.py         # Webhook服务测试
...
- 📧 邮箱: sdk@autopay.com
- 💬 微信: AutoPay-SDK
- 📚 文档: https://docs.autopay.com/sdk/python
- 🐛 Bug报告: https://github.com/autopay/python-sdk/issues
- 💡 功能建议: https://github.com/autopay/python-sdk/discussions
...
**Made with ❤️ by AutoPay SDK Team**