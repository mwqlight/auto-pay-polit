...
import { AutoPay } from '../src/AutoPay';
...
describe('AutoPay SDK', () => {
  ...
  describe('初始化', () => {
    test('应该能够创建AutoPay实例', () => {
      const autoPay = AutoPay.create('test-api-key', 'test-secret-key');
      expect(autoPay).toBeInstanceOf(AutoPay);
    });
    ...
    test('应该能够从配置对象创建实例', () => {
      ...
      const autoPay = AutoPay.fromConfig(config);
      expect(autoPay).toBeInstanceOf(AutoPay);
    });
  ...
  describe('错误处理', () => {
    test('应该抛出错误当API密钥为空时', () => {
      expect(() => {
        AutoPay.create('', 'test-secret-key');
      }).toThrow('API密钥不能为空');
    });

    test('应该抛出错误当密钥为空时', () => {
      expect(() => {
        AutoPay.create('test-api-key', '');
      }).toThrow('密钥不能为空');
    });
